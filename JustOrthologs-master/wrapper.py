...
def parseArgs():
    ...
    parser = argparse.ArgumentParser(
        description="Provides a variety of tools which allows you to\n"
    ...
    parser.add_argument(
        "-g2",
        help="2nd GFF3 fasta file (gzip allowed with .gz)",
        action="store",
        dest="gff3_Two",
        required=False,
    )
    parser.add_argument(
        "-r1",
        help="1st Reference Genome (gzip allowed with .gz)",
        action="store",
        dest="ref_One",
        required=False,
    )
    parser.add_argument(
        "-r2",
        help="2nd Reference Genome (gzip allowed with .gz)",
        action="store",
        dest="ref_Two",
        required=False,
    )
    parser.add_argument(
        "-fa1",
        help="1st Fasta file (only used without --e)",
        action="store",
        dest="fasta1",
        required=False,
    )
    parser.add_argument(
        "-fa2",
        help="2nd Fasta file (only used without --e)",
        action="store",
        dest="fasta2",
        required=False,
    )
    parser.add_argument(
        "-e",
        help="Extract CDS regions from genomes",
        action="store_true",
        dest="extract",
        required=False,
    )
    parser.add_argument(
        "-r", help="Run JustOrthologs", action="store_true", dest="run", required=False
    )
    parser.add_argument(
        "-f",
        help="Filters genes based on annotations",
        action="store_true",
        dest="filter",
        required=False,
    )
    parser.add_argument(
        "-s",
        help="Sort FASTA file for running JustOrthologs",
        action="store_true",
        dest="sort",
        required=False,
    )
    ...
    parser.add_argument(
        "-d",
        help="For Distantly Related Species (only with --r)",
        action="store_true",
        dest="distant",
        required=False,
    )
    parser.add_argument(
        "-c",
        help="Combine Both Algorithms In JustOrthologs For Best Accuracy",
        action="store_true",
        dest="combine",
        required=False,
    )
    parser.add_argument(
        "-o", help="Output File for --r", action="store", dest="output", required=False
    )
    parser.add_argument(
        "-t",
        help="Number of Cores (only affects -r option)",
        action="store",
        dest="threads",
        type=int,
        required=False,
    ...
    if args.sort and args.gff3_One is None and args.fasta1 is None:
        ...
        sys.exit()
    if args.run and (
        (args.fasta1 is None and args.fasta2 is None)
        and (args.ref_One is None or args.ref_Two is None)
    ):
...
if __name__ == "__main__":
    ...
    if sort1 is not None and sort2 is not None:
        ...
        if args.run:
            ...
            prog = runJustOrthologs(
                sort1, sort2, threads, args.output, distant, combine
            )
            prog.communicate()
